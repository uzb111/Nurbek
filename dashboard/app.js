...
function routeChartSvg(steps, fieldLimit, outletLabel) {
  ...
  const dots = steps.map((step, index) => `<g><circle cx="${x(index)}" cy="${waterY(step.selectedWater)}" r="5" fill="#fff" stroke="#1174ee" stroke-width="3"><title>${index + 1}/${steps.length} tugun · ${escapeHtml(step.name)} · ${fmtInt.format(step.selectedWater)} m³</title></circle>${index === 0 || index === steps.length - 1 || index % 2 === 0 ? `<text x="${x(index)}" y="${waterY(step.selectedWater) - 11}" text-anchor="middle" fill="#0d5fc3" font-size="9" font-weight="800">${fmtInt.format(step.selectedWater)} m³</text>` : ""}</g>`).join("");
  const xLabels = steps.map((step, index) => {
    const label = index === 0 ? `Bosh manba · 1/${steps.length}` : index === steps.length - 1 ? `${escapeHtml(outletLabel)} · ${steps.length}/${steps.length}` : `Oraliq · ${index + 1}/${steps.length}`;
  ...
  return `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Bosh manbadan ${escapeHtml(outletLabel)}gacha va undan dalaga borgan suvning hisobiy kamayishi"><defs><linearGradient id="routeWaterArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#1687ff" stop-opacity=".28"/><stop offset="100%" stop-color="#1687ff" stop-opacity=".02"/></linearGradient><linearGradient id="routeLossArea" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="#f3a51b" stop-opacity=".05"/><stop offset="100%" stop-color="#f3a51b" stop-opacity=".3"/></linearGradient></defs><text x="${left}" y="18" fill="#52665a" font-size="10" font-weight="700">SUV HAJMI · m³</text><text x="${width - right}" y="18" text-anchor="end" fill="#0d6ed8" font-size="10" font-weight="800">DALAGA BORGAN ULUSH ${fmtDec.format(retained)}%</text>${grids}<polygon points="${waterArea}" fill="url(#routeWaterArea)"/><polygon points="${lossArea}" fill="url(#routeLossArea)"/><line class="route-limit-line" x1="${left}" y1="${limitY}" x2="${width - right}" y2="${limitY}" stroke="#7057d8" stroke-width="2.5" stroke-dasharray="9 7"/><polyline class="route-water-line" points="${waterPoints}" fill="none" stroke="#1174ee" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>${dots}${xLabels}</svg>`;
}
...
function renderRouteReport(properties) {
  ...
  const intermediateCount = Math.max(route.length - 2, 0);
  const transitionCount = Math.max(route.length - 1, 0);
  empty.hidden = true; data.hidden = false;
  document.querySelector("#route-report-title").textContent = `${outletLabel} orqali dalaga suv borgan`;
  document.querySelector("#route-report-subtitle").textContent = `1 bosh manba + ${intermediateCount} oraliq kanal/ariq tuguni + 1 yakuniy quloq = ${route.length} tugun · ${outletLabel} yo‘lning ${route.length}/${route.length}, oxirgi tuguni.`;
  document.querySelector("#route-metrics").innerHTML = `<div><span>Bosh manbadan ajratildi</span><strong>${selectedScenario ? `${fmtInt.format(selectedScenario.sourceShare)} m³` : "Ekin tanlang"}</strong><small>${escapeHtml(route[0])} · yagona bosh manba</small></div><div><span>Yo‘l bo‘yicha yo‘qotish</span><strong>${selectedScenario ? `${fmtDec.format(selectedScenario.lossPct)}%` : "—"}</strong><small>${selectedScenario ? `${fmtInt.format(selectedScenario.lossM3)} m³ · ${transitionCount} o‘tish` : "ekin tanlangach hisoblanadi"}</small></div><div><span>Dalaga borgan suv</span><strong>${selectedScenario ? `${fmtInt.format(selectedScenario.delivery)} m³` : "Ekin tanlang"}</strong><small>${escapeHtml(outletLabel)}dan dala uchun</small></div><div><span>Quloq xizmat hududi</span><strong>${fmtInt.format(selected.fields)} dala</strong><small>${escapeHtml(outletLabel)} · ${route.length}/${route.length} yakuniy tugun</small></div>`;
  const outletCodeExplanation = outlet.number ? `Undagi ${outlet.number} raqami` : "Uning nomi";
  document.querySelector("#route-explanation").innerHTML = `<strong>Quloq qayerda?</strong> <strong>${escapeHtml(outletLabel)}</strong>: ${outletCodeExplanation} marshrut bosqichi emas, chiqish nuqtasining kodidir. Bu quloq tanlangan yo‘lning <strong>${route.length}/${route.length} — oxirgi tuguni</strong>. Ungacha 1 ta bosh manba va ${intermediateCount} ta oraliq kanal/ariq tuguni bor; grafikdagi ${route.length} nuqta shular bilan yakuniy quloqni birga ko‘rsatadi. Binafsha punktir — manbada dala uchun ajratilgan suv, ko‘k chiziq esa ${transitionCount} ta o‘tishdan keyin dalaga borgan hisobiy suv. ${fmtInt.format(selected.fields)} dala — shu yakuniy quloqqa biriktirilgan dalalar soni; bu quloqlar soni emas.${largestDrop?.drop > 0 ? ` Eng katta yo‘nalish ajralishi ${largestDrop.from.index}- va ${largestDrop.to.index}-tugun oralig‘ida: ${fmtInt.format(largestDrop.from.fields)} daladan ${fmtInt.format(largestDrop.to.fields)} dala shu shoxda davom etadi.` : ""}`;
  document.querySelector("#route-stage-list").innerHTML = steps.map((step, index) => `<div class="route-stage-item"><span>${index === 0 ? `BOSH MANBA · 1/${steps.length}` : index === steps.length - 1 ? `YAKUNIY QULOQ · ${steps.length}/${steps.length}` : `ORALIQ TUGUN · ${index + 1}/${steps.length}`}</span><strong>${escapeHtml(step.name)}</strong><small>Shu yo‘nalishda ${fmtInt.format(step.fields)} dala · ${fmtInt.format(step.blocks)} yakuniy quloq</small></div>`).join("");
  ...
  chart.innerHTML = selectedScenario ? routeChartSvg(steps, selectedScenario.sourceShare, outletLabel) : "";
}
